...
(function(name, context, factory) {
  ...
  var util = {
    ...
    sum: function(func, end, options) {
      var sum = 0;
      options = options || {};
      for (var i = options.start || 0; i < end; i += options.inc || 1) sum += func(i) || 0;
    ...
    product: function(func, end, options) {
      var prod = 1;
      options = options || {};
      for (var i = options.start || 0; i < end; i += options.inc || 1) prod *= func(i) || 1;
    ...
    createArrayFromArgs: function(args, index, threshold) {
      ...
      for (var i = 0; i < threshold; i += 1) ret[i] = args(i >= index ? i + 1 : i);
